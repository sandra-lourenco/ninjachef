***Settings***
Documentation    Aqui temos todas as palavras chave de automação dos comportamentos
...
E que "${produto}" está cadastrada no meu dashboard
    ...
    &{headers}=       Create Dictionary    name=${produto}                            plate=Tipo    price=20.00
    ${image_file}=    Get Binary File      ${EXECDIR}/resources/images/produto.jpg    
    &{files}=         Create Dictionary    thumbnail=${image_file}
    &{payload}=       Create Dictionary    user_id=${token_cozinheiro} 
...
Então devo receber uma notificação de pedido desse produto
    ${mensagem_esperada}        Convert To String       ${email_cliente} está solicitando o preparo do seguinte ${produto}
    Wait Until Page Contains    ${mensagem_esperada}    5

E posso aceitar ou rejeitar esse pedidos
